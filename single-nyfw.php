		...
		<div id="content" class="col-md-8 col-sm-12 col-md-offset-2">
			...
			<article itemscope="itemscope" itemtype="http://schema.org/BlogPosting" itemprop="blogPost">
				...
				<!-- <ul class="single-social">
					<li><a class="share-facebook" href="#" target="popup" onclick="window.open('https://www.facebook.com/sharer/sharer.php?u=<?php the_permalink(); ?>','Share this post on Facebook','width=600,height=400')"><i class="fa fa-facebook"></i> &nbsp;<h4 class="nomarg">
					</h4></a></li>
			...
			<ul id="related-posts" class="row-fluid">
				...
				<ul class="container-fluid">
					...
					$args = array( 'category_name' => 'nyfw', 'post_type' => 'post', 'showposts' => 2, 'orderby' => 'date', 'order' => 'DESC', 'post__not_in' => array($this_post_ID) );
				...
				<ul class="alm-listing alm-ajax">
					...
					$cat_ID = $cat_ID[0]->cat_ID;
					$this_post = $post->ID;
					$args = array( 'category_name' => 'nyfw', 'offset' => 2, 'post_type' => 'post', 'showposts' => 4, 'orderby' => 'date', 'order' => 'DESC', 'post__not_in' => array($this_post) );
					$postslist = get_posts( $args );
					// $postslist = get_posts('tag=darling&numberposts=4&order=DESC&orderby=date&offset=0');
					foreach ($postslist as $post) : setup_postdata($post);
					?>
			    ...
			    <?php
					...
					// }
					echo do_shortcode('[ajax_load_more orderby="date" offset="6" category="nyfw" exclude="'.$wp_query->post->ID.'" button_label="Loading" cache="true" cache_id="1546845955"]');
			    ?>
			</ul>
		</div>
	</div>
