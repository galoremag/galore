	...
	<div class="row-fluid">
		...
		<div id="content" class="col-md-8 col-sm-12 col-md-offset-2">
			...
			<article itemscope="itemscope" itemtype="http://schema.org/BlogPosting" itemprop="blogPost">
				...
				<h1 class="updated_post_title"><?php the_title(); ?></h1>
...
<!-- 					<div class="catlinks"><?php the_category(); ?></div> -->
			...
			<ul id="related-posts" class="row-fluid">
				...
				<ul class="container-fluid">
					...
					$args = array( 'category_name' => 'fashion', 'post_type' => 'post', 'showposts' => 2, 'orderby' => 'date', 'order' => 'DESC', 'post__not_in' => array($this_post_ID) );
				...
				<ul class="alm-listing alm-ajax">
					...
					$cat_ID = $cat_ID[0]->cat_ID;
					$this_post = $post->ID;
					$args = array( 'category_name' => 'fashion', 'post_type' => 'post', 'offset' => 2, 'showposts' => 4, 'orderby' => 'date', 'order' => 'DESC', 'post__not_in' => array($this_post) );
					$postslist = get_posts( $args );
					// $postslist = get_posts('tag=darling&numberposts=4&order=DESC&orderby=date&offset=0');
					foreach ($postslist as $post) : setup_postdata($post);
					$categories = get_the_category($post->ID);
					...
					<li class="post">
						<article class="row-fluid">
							<div class="nopad col-sm-5">
 								<?php if( $categories ) : ?>
   				<div class="catlinks catlinks_post_feed" style="transform: rotate(<?php echo $cat_random_rotation_value ?>deg); position: relative; z-index: 2;">
	  			 <ul class="post-categories">
	   				<li><a class="updated_catlinks_anchor_tag" href="<?php echo $displayed_category_link ?>" rel="category tag"><?php echo 						$displayed_category_name ?></a></li></ul></div>
								<?php endif; ?>
								<div class="thumb">
			    ...
			    <?php
					...
					// }
					echo do_shortcode('[ajax_load_more orderby="date" offset="6" category="fashion" exclude="'.$wp_query->post->ID.'" button_label="Loading" cache="true" cache_id="6171335779"]');
			    ?>
			</ul>
		</div>
	</div>
