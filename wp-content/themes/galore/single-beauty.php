	   ...
	   <li><a class="updated_catlinks_anchor_tag" href="<?php echo $displayed_category_link ?>" rel="category tag"><?php echo $displayed_category_name ?></a></li></ul></div>
			...
			<ul id="related-posts" class="row-fluid">
				...
				<ul class="alm-listing alm-ajax">
					...
					$cat_ID = $cat_ID[0]->cat_ID;
					$this_post = $post->ID;
					$args = array( 'category_name' => 'beauty', 'offset' => 2, 'post_type' => 'post', 'showposts' => 4, 'orderby' => 'date', 'order' => 'DESC', 'post__not_in' => array($this_post) );
					$postslist = get_posts( $args );
					// $postslist = get_posts('tag=darling&numberposts=4&order=DESC&orderby=date&offset=0');
					foreach ($postslist as $post) : setup_postdata($post);
					$categories = get_the_category($post->ID);
			    ...
			    <?php
					...
					// }
					echo do_shortcode('[ajax_load_more orderby="date" offset="6" category="beauty" exclude="'.$wp_query->post->ID.'" button_label="Loading" cache="true" cache_id="2175949131"]');
			    ?>
			</ul>
		</div>
	</div>
